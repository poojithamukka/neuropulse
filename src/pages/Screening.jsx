...
// ── Word pool for Alzheimer's test (randomized each time) ──
const WORD_POOL = [
...
// ── MediaPipe loader — tries CDN, resolves false if it fails ──
let _mediaPipeLoading = false;
let _mediaPipeLoaded  = false;
function loadMediaPipeScript() {
  return new Promise((resolve) => {
    if (_mediaPipeLoaded && window.FaceMesh) { resolve(true); return; }
    if (_mediaPipeLoading) {
      const poll = setInterval(() => {
        if (window.FaceMesh) { clearInterval(poll); _mediaPipeLoaded = true; resolve(true); }
      }, 200);
    ...
    // pinned version for reliability
    script.src = 'https://cdn.jsdelivr.net/npm/@mediapipe/face_mesh@0.4.1633559619/face_mesh.js';
    script.crossOrigin = 'anonymous';
    script.onload = () => {
      const poll = setInterval(() => {
        if (window.FaceMesh) { clearInterval(poll); _mediaPipeLoaded = true; _mediaPipeLoading = false; resolve(true); }
      }, 100);
      setTimeout(() => { clearInterval(poll); _mediaPipeLoading = false; resolve(false); }, 8000);
    };
    script.onerror = () => { _mediaPipeLoading = false; resolve(false); };
...
export default function Screening() {
  ...
  // ── HELPER: draw mirrored video onto canvas ──
  function drawMirroredVideo(canvas, video) {
  ...
  // ── HELPER: draw contour (landmarks are already mirror-adjusted) ──
  function drawContour(ctx, landmarks, W, H, indices, color, lineWidth) {
    ...
    indices.forEach((idx, i) => {
      ...
      const x = (1 - landmarks[idx].x) * W;  // mirror x to match flipped video
  ...
  async function startStrokeCam() {
    ...
    try {
      ...
      // Wait for video metadata before playing — critical fix
      await new Promise(res => { vid.onloadedmetadata = () => res(); });
      await vid.play();

      const mpLoaded = await loadMediaPipeScript();

      if (mpLoaded) {
        // MediaPipe available — full landmark mode
        ...
        // Fallback — show plain mirrored video on canvas without landmarks
        toast.info('Running in basic mode (no landmark model loaded)');
        const canvas = strokeCanvasRef.current;
        const loop = () => {
          if (strokeVideoRef.current && canvas && strokeVideoRef.current.readyState >= 2) {
            canvas.width = strokeVideoRef.current.videoWidth || 640;
            canvas.height = strokeVideoRef.current.videoHeight || 480;
            drawMirroredVideo(canvas, strokeVideoRef.current);
          }
          strokeLoopRef.current = requestAnimationFrame(loop);
        };
        strokeLoopRef.current = requestAnimationFrame(loop);
      ...
      console.error('Stroke cam error:', err);
  ...
  function onStrokeResults(results) {
    ...
    // ✅ FIXED: draw mirrored video using translate+scale (not just scale)
    drawMirroredVideo(canvas, video);
    ...
    // Draw all 468 dots (x mirrored to match flipped video)
    ...
    ctx.fillStyle = 'rgba(0,0,0,0.6)'; ctx.fillRect(8, 8, 240, 80);
    ctx.font = 'bold 13px monospace';
    ctx.fillStyle = '#00ff88'; ctx.fillText(`Landmarks: ${landmarks.length} / 468`, 14, 28);
    ctx.fillStyle = '#ccc'; ctx.fillText(`Mouth asymmetry: ${mouthAsymmetry.toFixed(1)}%`, 14, 48);
    ctx.fillText(`Brow asymmetry:  ${browAsymmetry.toFixed(1)}%`, 14, 66);
    ctx.fillStyle = overallAsym > 15 ? '#ff4444' : '#00cc66';
    ctx.fillText(overallAsym > 15 ? '⚠ Asymmetry Detected' : '✓ Symmetry Normal', 14, 84);
  ...
  async function startEyeCam() {
    ...
    try {
      ...
      if (mpLoaded) {
        ...
        toast.info('Running in basic eye tracking mode');
        const canvas = eyeCanvasRef.current;
        const loop = () => {
          if (eyeVideoRef.current && canvas && eyeVideoRef.current.readyState >= 2) {
            canvas.width  = eyeVideoRef.current.videoWidth  || 640;
            canvas.height = eyeVideoRef.current.videoHeight || 480;
            drawMirroredVideo(canvas, eyeVideoRef.current);
          }
          eyeLoopRef.current = requestAnimationFrame(loop);
        };
        eyeLoopRef.current = requestAnimationFrame(loop);
      ...
      console.error('Eye cam error:', err);
      toast.error('Camera access denied. Please allow camera permission.');
      setEyePhase('ready');
    }
  }
  ...
  function onEyeResults(results) {
    ...
    // Eye center crosshairs
    const eyeGroups = [LEFT_EYE, RIGHT_EYE];
    eyeGroups.forEach(group => {
      let sx = 0, sy = 0, cnt = 0;
      group.forEach(idx => {
        if (idx < landmarks.length) { sx += (1 - landmarks[idx].x) * W; sy += landmarks[idx].y * H; cnt++; }
      });
      if (cnt === 0) return;
    ...
    // Gaze tracking from left eye center
    const leftEyePts = LEFT_EYE.filter(idx => idx < landmarks.length);
    if (leftEyePts.length > 0) {
      const sx = leftEyePts.reduce((s,idx) => s + (1-landmarks[idx].x)*W, 0) / leftEyePts.length;
      const sy = leftEyePts.reduce((s,idx) => s + landmarks[idx].y*H,      0) / leftEyePts.length;
      const gazeX = sx / W; const gazeY = sy / H;
    ...
    ctx.fillStyle = '#00ccff'; ctx.fillText(`Eye landmarks: ${LEFT_EYE.length + RIGHT_EYE.length}`, 14, 26);
  ...
  function stopEyeCam() {
    ...
    if (eyeFaceMeshRef.current) { try { eyeFaceMeshRef.current.close(); } catch(e){} eyeFaceMeshRef.current = null; }
  }
              ...
              {modules.map((module, i) => (
                <motion.button key={module.id}
                  ...
                  onClick={() => {
                    ...
                    if (module.id === 'alzheimers') { setMemoryWords(pickRandomWords()); setMemoryPhase('show'); setMemoryInput(''); setMemoryScore(null); }
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
              ...
              {memoryPhase === 'recall' && (
                <div className="text-center max-w-md mx-auto">
                  <p className="text-gray-400 mb-6">Type all the words you remember (separated by commas or spaces)</p>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
              ...
              {strokePhase === 'loading' && (
                <div className="flex flex-col items-center py-16">
                  ...
                  <p className="text-gray-400">Loading face analysis model...</p>
                </div>
              )}
              ...
              {(strokePhase === 'analyzing' || strokePhase === 'done') && (
                <div className="flex gap-6">
                  <div className="flex-1">
                    {/* ✅ FIXED: video hidden but still decoded; canvas fills the space */}
                    <div className="relative rounded-2xl overflow-hidden bg-black" style={{ aspectRatio: '4/3' }}>
                      ...
                      {landmarkCount > 0 && (
                        <div className="absolute bottom-3 left-3 bg-black/70 text-green-400 text-xs font-bold px-3 py-1 rounded-full">
                          ✅ {landmarkCount} / 468 landmarks
                        </div>
                      )}
                      {landmarkCount === 0 && strokePhase === 'analyzing' && (
                        <div className="absolute bottom-3 left-3 bg-black/70 text-yellow-400 text-xs font-bold px-3 py-1 rounded-full">
                          🔍 Detecting face...
                        </div>
                      )}
                    </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
              ...
              {(eyePhase === 'tracking' || eyePhase === 'done') && (
                <div className="flex gap-6">
                  <div className="flex-1">
                    <div className="relative rounded-2xl overflow-hidden bg-black" style={{ aspectRatio: '4/3' }}>
                      {/* ✅ FIXED: same hidden video pattern */}
                      <video
                        ref={eyeVideoRef}
                        playsInline muted autoPlay
                        style={{ position: 'absolute', width: 1, height: 1, opacity: 0, pointerEvents: 'none' }}
